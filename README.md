...
- POST `/api/create-checkout` — creates a Supabase order and Paystack transaction initialization
- POST `/api/contact-submit` — saves contact submissions
- POST `/api/newsletter-subscribe` — subscribes to newsletter
- POST `/api/paystack-webhook` — Paystack webhook handler (verifies signature, records payment, updates order status)
